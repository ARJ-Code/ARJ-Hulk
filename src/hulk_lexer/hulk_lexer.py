...
numeric_constants_regex = [(t.upper(), Regex(t)) for t in NUMERIC_CONSTANTS]
defined_functions_regex = [(t.upper(), Regex(t)) for t in DEFINED_FUNCTIONS]

num_regex = Regex('0|([1-9][0-9]*)((\.|e\+|e\-|e)[0-9]+)?')

string_regex = Regex('"(\\\\[tnr"\']|[^"\\\\])*"|\'(\\\\[tnr\'"]|[^\'\\\\])*\'')

identifier_regex = Regex('(_|[a-zA-Z])(_|[a-zA-Z0-9])*')

ignore_regex = Regex('( |\n|\t)+|//[^\n]*\n|/\*[^(\*/)]*\*/')

hulk_lexer = Lexer(reserved_words_regex +
